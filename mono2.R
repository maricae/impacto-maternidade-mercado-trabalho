...
mono$`QT DIAS AFAS2` <- (mono$`QT DIAS AFAS`)^2
rm(painelid_0709_1)
...
## Modelo Pooled
reg.pooled <- plm(ln_salario ~ sexo + licença + sexo:licença + afastamento_licença + grau_instr + temp_empr + idade + idade2 , data = painel, model='pooling')
summary(reg.pooled)

## Modelo Efeitos Fixos
reg.ef <- plm(ln_salario ~ sexo + licença + sexo:licença + afastamento_licença + grau_instr + temp_empr + idade + idade2 , data = painel, model='within')
summary(reg.ef)

## Modelo de Efeitos Aleatórios
reg.ea=plm(ln_salario ~ sexo + licença + sexo:licença + afastamento_licença + grau_instr + temp_empr + idade + idade2 , data = painel, model="random", random.method = "walhus")
summary(reg.ea)
